...
int init(int numberOf)
{
    ...
    int len = 0;
    int countStart = 0;
    printf("Enter %d figure\nexample: triangle(x1 y1, x2 y2, x3 y3, x4 y4), circle(x1 y2, radius)\n", numberOf);
    ...
    else if ((input[0] == triangleEx[0]) || (input[0] == triangleEx[0] - ' '))
    {
    ...
    {
        printf("Please, enter the correct figure! Expected: 'circle' or 'triangle'.\n");
        return 0;
    }
    return 0;
...
int figure_check(char a[], char b[])
{
    for(int i = 1; a[i] != '('; i++){
        if((a[i] == b[i]) || (a[i] == b[i] - ' ')){
            ...
            printf("Undefined figure. Maybe you put something between command and '(', check it.\n");
            return 0;
        }
    }
    return 0;
...
int converter(char input[], int len, int countStart, int answer_point)
{
    ...
    {
        if ((input[j] == ',') || (input[j] == '.') || (input[j] == ' ') || ((input[j] >= '0') && (input[j] <= '9')))
            continue;
        else
        {
    ...
    if (answer_point == 1)
    {
        ...
        for(countStart += 1; countStart < len; countStart++){
            if(input[countStart] == ' ' || input[countStart] == ',' || input[countStart] == ')')
            {
                continue;
            } else {
                if(input[countStart + 1] == ' ' || input[countStart + 1] == ',' || input[countStart + 1] == ')')
                {
    ...
    if(answer_point == 1)
    {
        printf("Circle's x = %.2lf, y = %.2lf\n", cir_obj.center.x, cir_obj.center.y);
    ...
    if(answer_point == 2)
    {
        if((tr_obj.p[0].x != tr_obj.p[3].x) && (tr_obj.p[0].y != tr_obj.p[3].y)){
            printf("First and last coordinate can't be different!\n");
            return -1;
        }
        
        for(int k = 0; k < 4; k++){
            printf("Triangle's x%d = %.2lf, y%d = %.2lf\n", k+1, tr_obj.p[k].x, k+1, tr_obj.p[k].y);
        }
...
{
    double first_side = sqrt(SQR((tr->p[1].x - tr->p[0].x)) + SQR((tr->p[1].y - tr->p[0].y)));
    double second_side = sqrt(SQR((tr->p[2].x - tr->p[1].x)) + SQR((tr->p[2].y - tr->p[1].y)));
    double third_side = sqrt(SQR((tr->p[3].x - tr->p[2].x)) + SQR((tr->p[3].y - tr->p[2].y)));
    tr->perimetr = first_side + second_side + third_side;
    double halfp = tr->perimetr / 2;
    tr->squre = sqrt(halfp*(halfp - first_side)*(halfp - second_side)*(halfp - third_side));
    return tr;
